...
-- get commands
hcmd.g_jack = [[cat /proc/asound/card1/codec#0 | grep 'Pin-ctls:' | head -3 | tail -1 | grep -c OUT; exit ${PIPESTATUS[0]}]]
hcmd.g_ismuted = [[pactl list sinks | grep "^\s\+Mute: yes" | awk '{print $2}'; exit ${PIPESTATUS[0]}]]
...
hcmd.sg_lowervol = {"bash", "-c", "pactl set-sink-volume 0 -"..vol_step.."% >/dev/null 2>&1;".._volume}
hcmd.sg_raisevol = {"bash", "-c", "pactl set-sink-volume 0 +"..vol_step.."% >/dev/null 2>&1;".._volume}
hcmd.sg_togglemute = {"bash", "-c", _togglemute..';'.._volume}
